...
class Matcher:
    ...
    # our matching function that returns extraction
    # as matching the gold standard if the F1 score
    # is above a certain threshold
    @staticmethod
    def lexicalMatchNew(ref, ex, ignoreStopwords, ignoreCase):
        sRef = ref.bow().split(' ')
        sEx = ex.bow().split(' ')
        count = 0
        # for w1 in sRef:
        ...
        #       being too long
        coverage = float(count) / len(sRef)
        precision = float(count) / len(sEx)

        if (coverage + precision) == 0:
            print("coverage + precison is 0")
            return False

        # return precision > Matcher.PRECISION_THRESHOLD

        f1_score = (coverage * precision) / (coverage + precision)
        # threshold for F1 score, compute F1 score from other function (tuple match)

        return f1_score > Matcher.F1_THRESHOLD
    ...
    def linient_tuple_match(ref, ex, ignoreStopwords, ignoreCase):
        ...
        for i in range(len(ref.args)):
            ...
            predicted_words = ex.args[i].split()
            p = []
            #print("before: "+str(predicted_words))
            for w in predicted_words:
                if w == ".":
                    ...
                    p.append(w)
            predicted_words = p
            #print("after: "+str(predicted_words))
            precision[1] += len(predicted_words)
            matching_words = 0
            for w in gold_words:
                if w in predicted_words:
    ...
    F1_THRESHOLD = 0.25  # f1 threshold for precision 1 and recall 0.5
    PRECISION_THRESHOLD = 0.8  # new precision threshold introduced
